...
import { useToast, Input, Link, Button, Heading, Text, ButtonGroup, AlertDialog, 
    AlertDialogBody, AlertDialogFooter, AlertDialogHeader, AlertDialogContent, AlertDialogOverlay, useDisclosure, Image } from '@chakra-ui/react';
            ...
            <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-4">
                <div className="max-w-md w-1/2 space-y-8">
                    <div>
                        ...
                        <Heading className="mt-6 text-center" size="xl">Create a new account</Heading>
                        <Text fontSize="sm" className="mt-2 text-center">
                            Or{' '}
                            <Link href="login" color="teal.500">sign in</Link>
                        </Text>
                    </div>
                    <form className="mt-8 space-y-4">
                        <input type="hidden" name="remember" defaultValue="true" />
                        <Input placeholder="Display name" type="text" onChange={(e) => {
                            setDisplayName(e.target.value);
                        }} />
                        <Input placeholder="Email Address" type="email" onChange={(e) => {
                        ...
                        <Input placeholder="Password" type="password" onChange={(e) => {
                            ...
                            if (e.key === "Enter") {
                                ...
                                createUserWithEmailAndPassword(auth, emailAddr, password)
                                .then(async (userCredential: any) => {
                                    const user = userCredential.user;

                                    updateProfile(user, {
                                        displayName: displayName
                                    }).then(() => {console.log("Updated display name")}).catch((error) => {console.log(error)});
                    ...
                    <div className="w-full flex items-center justify-between">
                        ...
                        <div className="flex justify-around w-full">
                            <ButtonGroup variant="solid" spacing="4">
                                <Button aria-details="github login" height="12" px="14" onClick={(e) => {
                                    // It redirects but doesn't do anything else.

                                    signInWithRedirect(auth, githubProvider)
                                    .then(async (result) => {
                                        const credential = GithubAuthProvider.credentialFromResult(result);

                                        console.log("Done"); // <- This doesn't print
                                    }).catch((error) => {
                                        const errorCode = error.code;
                                        const email = error.email;
                                        const credential = GithubAuthProvider.credentialFromError(error);

                                        console.log(errorCode) // <- This also doesn't print
                                    })
                                }}>
                                ...
                                <Button height="12" px="14" onClick={(e) => {
